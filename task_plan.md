...
岗位候选人页体验优化（布局、展开空态、面包屑跳转）已完成并已推送；后续按需做浏览器验收与增量需求。
...
- [x] 展开行「候选人跟踪 / 候选人评价」空态文案：无跟踪记录、无候选人评价
- [x] 过滤系统 AUTO_MATCH 自动匹配事件，避免误展示为人工跟踪
- [x] 全站面包屑上级路径可点击跳转（`renderCrumbsHtml` + `crumbHrefMap`）
- **Status:** complete
