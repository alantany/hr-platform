...
## 2026-07-18 登录页视觉改版完成记录

- [x] 登录页 markup 契约测试
- [x] 浅色分栏页面与 XL 六边形 logo
- [x] 全局 `styles.css` 登录覆盖规则收口
- [x] 回归测试与任务记录更新
...
产品说明手册 Word 版已生成（`outputs/猎头招聘交付系统-产品说明手册.docx`）。维护流程：**改代码/文档 → 更新三份 MD → commit → push**。
...
- [x] 修复求职者数据池列表页展开折叠行中，候选人跟踪显示为空（因误用 followUps 且未获取 tracking 记录）和新录入的薪资记录无法显示的 Bug
- [x] 扩充标签管理（候选人、岗位、项目、客户）的可作为标签的字段可选范围
- [x] 修复用户管理中“创建用户”与“编辑用户”弹窗容器在小屏幕下因缺少垂直滚动支持而被截断拉不到底部的 Bug
- [x] 修复超级管理员点击顶部消息数字跳转通知页 notifications.html 提示无访问权限并拦截的 Bug
- [x] 首页数据看板顶部的五大统计卡片新增点击下钻与跳转对应功能模块页面的功能
- [x] 客户管理页面顶部的四大统计卡片新增点击下钻与跳转对应功能模块页面的功能
- [x] 岗位管理页面顶部的四大统计卡片支持跳转项目管理与就地实时过滤高紧急度岗位
- [x] 项目管理页面顶部的四大统计卡片支持跳转岗位管理与就地实时过滤招聘状态项目
- [x] 全局功能模块页面的数字看板统计卡片完成交互改造（涉及求职者数据池、评价体系、操作日志、通知中心、Recruit 岗位管理与岗位发布页面）
- [x] 修复求职者数据池中已锁定候选人参数不匹配与今日新增无日期比对导致过滤卡片点击失效的 Bug
- [x] 修复客户管理页面中“累计交付人数”由于错误绑定空数据表导致统计数量为 0 与质保期在库数据不一致的 Bug
- [x] 成功实施了“全员共享候选人池 + 同级流转阻断 + 无限级下属CTE穿透”权限与隔离控制体系
- [x] 权限管理页功能权限配置升级为多选角色复选框组，并在后端新增了 `DELETE /api/role-permissions` 解除关联接口，支持批量保存 Promise 并发比对
- [x] 角色管理页重构为一站式“编辑与授权”弹窗，平铺展示系统所有预设的权限标签复选框，并支持全自动分类联动和批量保存更新
- [x] 岗位发布、岗位列表、每日任务三个菜单标签权限补齐到功能权限矩阵与角色分配矩阵中，实现中英映射转换与页面分类展示
- **Status:** done

### Phase 16 - 岗位分配确认任务（已完成）

- [x] 新增岗位分配任务状态模型与迁移
- [x] 分配岗位时生成待确认任务，接受后才启用数据权限
- [x] 操作员支持接受/拒绝，组长收到处理回执
- [x] 首页“待处理需求”改为“待办岗位”并统计当前用户待确认数量
- [x] 通知中心增加岗位待办列表及处理入口
- [x] 补充自动化测试、测试数据清理并完成全量回归
- **Status:** complete

### Phase 17 - 首页推荐日历统计口径修正（已完成）

- [x] 确认原日历实际优先统计交付记录
- [x] 将现有日历数字改为“已推荐”候选人数
- [x] 操作员按本人、组长按本人及全部下属过滤推荐人范围
- [x] 保持原日历组件、布局和交互不变
...
### Phase 22 - 候选人锁定与推荐状态硬联动（已完成）

- [x] 推荐创建、推进、失败终态、录用和删除统一同步候选人摘要状态
- [x] 禁止候选人锁定状态被创建、编辑或独立锁定接口手工修改
- [x] 移除候选人页面、岗位候选人页和质保页的手工状态入口
- [x] 新增 Alembic 存量数据修复并成功应用到 PostgreSQL
- [x] 补齐状态联动回归测试和桌面/390px 浏览器验收
- **Status:** complete

### Phase 23 - 候选人三维状态标签统一（已完成）

- [x] 将锁定状态、流程状态、质保状态拆分为三个独立字段维度
- [x] 锁定仅由岗位推荐关系决定，未录用不自动解除岗位锁定
- [x] 移除旧“待推荐/已录用”流程状态，入职只由详情页手工确认
- [x] 质保只由已入职记录及入职日期驱动，统一为六个月
- [x] 简历池、岗位候选人和质保候选人列表统一展示标签管理中启用的字段
- [x] 修复启动和测试清理覆盖用户标签配置的问题
- **Status:** complete

### Phase 24 - 面试评价模块重构（已完成）

- [x] 将旧有【薪资/福利/入职条件跟踪表】彻底更名为【面试评价记录】，并同步重构相关的添加、编辑和展示弹窗。
- [x] 评价等级（优秀/良好/一般/较差/不合格）自动联动映射到分值 1-5 分，不需要手动重复点选。
- [x] 前端优化多层级候选人 ID 异步提取，实现添加、编辑或物理删除评价后，详情弹窗内无缝动态局部重绘。
- [x] 调整菜单排序，将评价体系移到侧边栏最后，移除了首页和随访记录等处的冗余入口。
- **Status:** complete

### Phase 25 - 操作员角色岗位创建与权限隔离调整（已完成）

- [x] 限制操作员创建岗位权限，前端隐藏按钮，后端 add_position 接口拦截并抛出 403 错误。
- [x] 允许组长多选指派组员（Checkbox Checklist 形式，并且自动回显当前岗位已分配的人员列表）。
- [x] 将创建岗位与指派权限按钮收口至“组长”角色独享，管理员作为兜底。
- [x] 放宽客户、项目和岗位的可见性判定，使其对于组长和操作员均全局完全可见。
- [x] 保持并细化操作员的推荐权限隔离与导出/详情权限隔离。
- **Status:** complete

### Phase 26 - 岗位规则、JD 与候选人页体验（已完成）

- [x] 岗位锁定上限固定为 10，去掉创建时关键词自动匹配与客户→项目二级选择
- [x] 项目完成口径：各岗位已入职人数等于 hiring_count 时项目同步为「招聘完毕」
- [x] 简历池新增「未面试」「自己锁定」筛选；客户列表增加跳转项目按钮
- [x] 复用 `positions.description` 接入岗位 JD 创建/编辑/列表/候选人页展示
- [x] 岗位候选人页布局重排：合并卡片、副标题动态化、移除处理时限标签
- [x] 修复简历池批量推荐「无权访问该候选人」：未锁定简历仅校验岗位权限
- [x] 岗位候选人展开面板改为「候选人评价」；任务看板导航分组调整
- [x] 新增 `test_position_lock_quota_and_project_complete.py` 与推荐权限回归测试
- **Status:** complete

### Phase 27 - 岗位候选人页交互 polish（已完成）

- [x] 展开行「候选人跟踪 / 候选人评价」空态文案
- [x] 过滤系统 AUTO_MATCH 自动匹配事件
- [x] 全站面包屑上级路径可点击跳转
- [x] 岗位列表去掉 JD 行内展示（JD 仅在创建/编辑与岗位候选人页展示）
- [x] 展开面板卡片化；评价无文字备注时不显示「暂无评价内容」
- **Status:** complete

### Phase 28 - JD单生成岗位（已完成）
