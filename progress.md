...
- **智能解析无缝持续 Loading 状态 (Persistent Toast)**：
  - 重构 `app.js` 全局 Toast 框架，支持 `persistent` 持续常驻与 `isLoading` 状态。
  - 点击【智能解析 JD】按钮后，右下角弹出持续提示：`🤖 AI 正在对 JD 文本进行深度智能解析与关键词提取...`（内置动态 `...` 波浪跳跃 CSS 动画），**绝不会在解析中途突然消失产生空白感**。
  - 解析全程按钮处于 `disabled` 防重状态，直到后端 API 返回成功或失败后，才自动平滑销毁 Loading Toast 并展示最终结算 Toast。
- **页面副标题与描述话术业务化重构**：
  - 彻底干掉 `PostgreSQL`、`recruit.job_postings` 等技术硬词，替换为面向 HR/猎头的业务化高价值表达。
