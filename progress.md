...
## 2026-06-28 (已完成 - 岗位树操作按钮间距与悬浮窗 UI 自适应优化)

- **UI 优化与功能完善**：
  1. **按钮区域划分与拉开距离**：岗位操作栏的专属操作按钮（编辑、状态、删除）与候选人专属操作按钮（添加候选人、批量移除）在视觉上容易混淆。对 `renderPositionTreeItem` 中的 `.table-actions` 改用 flex 布局，并在二者之间增加一条浅灰色分割竖线（`border-left` 与 margin），使候选人相关按钮向右侧聚拢，清晰划定职责。
  2. **悬浮窗 UI 自适应（避免折行与臃肿）**：当以 `select_mode=1` 嵌入 iframe 时，由于最大宽度为 1200px 且受限于 modal 填充，触发了 `styles.css` 中 `@media (max-width: 1200px)` 的媒体查询，导致原本水平排列的表格列与搜索框堆叠成单列块，极其不美观。
  3. **样式重置覆盖**：在 `isSelectMode` 样式注入中新增覆盖逻辑：
     * 隐藏不需要的全局 `.metrics` 统计卡，节约黄金可视区高度。
     * 强制指定表格 `.table-head, .table-row` 宽度比重 `grid-template-columns: 48px 1.5fr 1fr 1fr 1fr 1fr 0.8fr 1fr !important`，使其在大容器中完美保持横向表格形态，杜绝折行。
     * 将搜索与筛选面板从纵向堆叠重置为并排网格，并将两栏面板内的下拉项（学历、年龄、经验）强制改为紧凑的 `flex-direction: row` 并排流式布局，使页面整体空间利用率提升 60% 以上。
