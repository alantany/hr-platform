...
## 2026-06-30（已完成 - 数据权限隔离体系大版本开发：全员共享人才库 + 敏感数据脱敏 + 过程隐私阻断 + CTE递归下属树）
...
- **公共人才库共享开放**：移除人才库中对 Candidate 列表和详情的 owner 拦截限制，使所有顾问共享公司人才资源。
- **敏感数据脱敏与过程隔离**：
  - 非归属人/非下属查看求职者时，手机和邮箱通过正则表达式星号脱敏（不影响数据库真实数据）。
  - 对这几类候选人身上发生的岗位推荐进度、面试记录、Offer 约定和薪金进行了基于 `operator_user_id` 的同级彻底阻断隔离，但对其上级组长和总监保留无感穿透可见。
...
<!-- trigger update for pre-commit (v12) -->
