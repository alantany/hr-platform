...
## 2026-07-28（完成 - 页面副标题与描述话术业务化重构）

- **全面清理技术黑话与底层表名**：
  - 彻底干掉 `PostgreSQL`、`recruit.job_postings`、`Recruit` 等面向底层数据库的技术硬词，全面替换为面向 HR / 猎头 / 操作员的高价值业务文案。
  - **岗位发布**：`配置与发布自动化招聘岗位需求，支持 AI 智能解析 JD 提取精准匹配关键词与岗位画像。`
  - **每日任务**：`实时查看每日自动化人才抓取、打招呼沟通、索要简历及下载履约的数据统计。`
  - **岗位列表**：`集中管理与维护当前发布的所有招聘岗位、搜索关键词及自动化抓取生效状态。`
  - **解析关键词**：`配置 AI 智能解析 JD 时重点关注的关键词矩阵，提升人才匹配与精筛精准度。`
- **岗位列表 Grid 比例与文本对齐重构**：
  - 移除 `recruit-job-list.html` 中的内联软性比例，使用专有 CSS 类 `.recruit-job-table` 统一约束表头 `table-head` 与内容行 `table-row`。
