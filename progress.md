...
## 2026-07-28（完成 - 搜索关键词硬性能力拆分排头与点标签一键填入落地）

- **AI 大模型硬性必备能力拆分与最高优先级排头**：
  - 更新 `crud.py` 中的 DeepSeek LLM 系统 Prompt，明确指令 AI 必须优先提取 JD 中【候选人必须具备的硬性能力/资质/工具/硬门槛】（自动排除表单已有的年龄和学历）。
  - 若有多个必备硬性能力，拆分为多个独立短关键词，**强制作为第 1 优先级排在 `search_keywords` 最前端**！第 2 优先级才是优先技能与行业词。
- **推荐关键词“点击一键自动填入”**：
  - 更新 `recruit-job-publish.html` 中的推荐关键词标签云，标签具备鼠标悬浮提示“点击自动追加至搜索框”与手型动画。
  - 点击任何推荐标签，**自动将关键词填入/追加至左侧【搜索关键词】框**（自动空格分隔与去重），省去复制粘贴动作。
- **保留并强化【硬性要求】与【优先要求】分层机制**：
  - **🔒 岗位硬性要求（一票否决/必具备条件）**：清晰展示 JD 中强要求的年龄门槛、学历门槛、必备证书，指导机器人/筛选逻辑做第一轮硬性切分；
  - **⚡ 岗位优先要求（优先条件与精筛权重）**：清晰展示 JD 中的岗位契合 (20%)、行业经验 (10%)、核心技能 (30%)、工作年限 (20%)、获奖与优先项 (20%)，并匹配对应的权重分值用于二次深度评分。
...
- **后端 API & 大模型集成**：在 `crud.py` 与 `main.py` 中新增 `POST /api/recruit/jobs/parse-jd` 接口；完整接入与系统“简历池 AI 搜索”一致的大模型 API 配置（`get_deepseek_config()` / `OpenAI` 客户端），实现真实 DeepSeek/LLM 智能提取，同时具备规则引擎高可靠兜底；在岗位发布与查询路由中完美绑定 `job_profile` 数据。
- **前端 UI & 鉴权修补**：将 `parseRecruitJd` 正式接入 `frontend-api.js` 统一网络层，自动继承并传递权威 `hr_token` 凭证，彻底解决了由于本地 Token 键名不一致导致的 401 `"账号不可用"` 报错问题。
...
## 2026-07-20（完成 - 品牌中线调细）

- `.brand-divider` 宽度 `2px → 1px`；缓存 `styles.css?v=20260720-brand-line-thin`。

## 2026-07-20（完成 - 侧栏品牌按设计稿：中线分隔）

- 左：Z 标 + 白字「职易达」；中：纵向渐变亮线（中间亮、两端淡 + 青光晕）；右：「AI一站式招聘交付」加大加粗。
- 缓存 `?v=20260720-brand-divider`。

## 2026-07-20（完成 - 侧栏品牌改为左图右双行）

- 修正「说不清的不对」：副标题原先只对齐图标、忽略下方「职易达」，视觉上飘在半空。
- 改为左 Z 标、右双行（上「职易达」白字加粗，下「AI一站式招聘交付」略淡）；缓存 `?v=20260720-brand-stack`。
...
- 重处理：去掉棋盘灰白像素为真透明，按图文空隙裁完整 Z，导出 160/320 mark；缓存 `?v=20260720-logo-fix`。
- 说明：网页上「矢量缩小」正解是导出 SVG，用 CSS 缩放；当前无 SVG 源，暂用透明 PNG。
...
- 白底圆角托盘 + 轻阴影；标题拆为「职易达」加粗与「AI一站式招聘交付」。
- 缓存：`styles.css?v=20260720-brand-logo`、`app.js?v=20260720-brand-logo`。

## 2026-07-20（完成 - 侧栏品牌单行）

- 去掉副标题「人力资源招聘管理系统 v3.0」，只保留「职易达 · AI一站式招聘交付」。
- 侧栏略加宽至 286px，标题 13px + `nowrap`，避免换行堆叠；缓存 `styles.css?v=20260720-brand-line`。

## 2026-07-20（完成 - 侧栏品牌文案）

- 侧栏左上角主标题由「AI招聘管理平台」改为「职易达 · AI一站式招聘交付」（`app.js` brand）。
...
- 文字改深蓝灰以保证浅底对比；缓存 `styles.css?v=20260720-topbar-light`。
...
- 新建 `outputs/manual-screenshots/` 与 README，约定截图命名与存放路径。

## 2026-07-19（完成 - 产品说明手册）

- 新增 `outputs/产品说明手册.md`：覆盖产品定位、系统边界、角色权限、业务对象、端到端流程，以及全部功能模块介绍（交付主链、Recruit 岗位发布、权限体系、AI/系统配置、开发工具）。
- 与 `outputs/用户操作手册.md` 分工：本手册偏 What/Why；操作手册偏 How。
- 依据页面导航、`app.js` 元信息与代码库功能清单整理。

## 2026-07-19（完成 - 顶栏与正文间距加大）

- `.topbar` 下边距：桌面 `24px → 40px`，中屏 `32px`，窄屏 `24–28px`；全局生效。
- 缓存 `styles.css?v=20260719-topbar-gap`。

## 2026-07-19（完成 - 全站顶栏蓝色背景条）

- `.topbar` 改为冰蓝横向渐变 `#0B4F8A → #0284C7 → #0EA5E9`，负边距拉满内容区宽度。
- 面包屑 / 标题 / 副文案改为白字；右上用户胶囊与图标按钮保持白底以保证对比。
- 同步 900/640 响应式边距；缓存 `styles.css?v=20260719-topbar-blue`。

## 2026-07-19（完成 - 顶栏显示登录用户名）

- 右上角用户卡由 `full_name` 改为 `username`。
- 缓存 `app.js?v=20260719-top-username`。

## 2026-07-19（完成 - 顶栏用户区精简）

- 用户卡只显示姓名；设置图标改为齿轮，避免被看成亮度/太阳。
- 缓存 `?v=20260719-top-user2`。

## 2026-07-19（完成 - 用户信息移至右上角）

- 侧栏底部用户区移除；顶栏右侧展示头像/姓名/角色，并保留设置、退出图标按钮。
- 缓存 `app.js?v=20260719-top-user`。

## 2026-07-19（完成 - 侧边栏 AI 冰蓝科技色）

- 渐变 `#E8F4FF → #B8D9F5 → #5B9FD4`；文字 `#0B2A4A`；选中青蓝 `rgba(14,165,233,.18)` / `#0284C7`。
- 缓存 `?v=20260719-sidebar-ai`。

## 2026-07-19（完成 - 侧边栏渐变加深一档）

- 侧栏渐变调整为 `#DCECF8 → #BFDDF2 → #8FBFDF`，仍保持上浅下深。
- 缓存 `?v=20260719-sidebar-mid`。

## 2026-07-19（完成 - 侧边栏浅蓝渐变）

- 侧栏背景改为上浅下深：`#F7FBFF → #E3F0FB → #C5DDF5`；文字/图标改深蓝灰以便对比。
- 选中/悬停用半透明深蓝高亮；底栏用户区与按钮同步浅色描边。
- 缓存 `?v=20260719-sidebar-light`。
...
- 缓存 `?v=20260719-month-solid3d`。

## 2026-07-19（完成 - 首页日历/月度看板实体悬浮）

- 「推荐日历」「月度数据看板」大框对齐顶部 KPI solid-3d：微蓝渐变、`#c2def8` 边框、`14px` 圆角、左右下硬阴影。
- 缓存 `?v=20260719-panel-solid3d`。

## 2026-07-19（完成 - 列表页 KPI 对齐首页实体卡）

- 客户/项目/候选人/任务看板/岗位发布/岗位列表/每日任务/操作日志顶部卡统一为首页 solid-3d：渐变、硬阴影、`14px` 圆角、`93px` 高、数字 `#003fa3`。
- 去掉上述页面卡片内全部小字（`metric-label` / trend）。
- 缓存 `?v=20260719-list-solid3d`。
...
- 内边距改为 `12px 16px 10px`、间距 `4px`，避免在 `93px` 高度内顶边。
- 缓存 `?v=20260719-kpi-fit`。
...
- 结构改为上标题、下「大数字 + 辅助文案」；数字色 `#003fa3`；内容区纯白底。
- 缓存 `?v=20260719-kpi-solid3d`；契约测试同步更新。

## 2026-07-19（完成 - 首页顶部 KPI 悬浮卡）

- 内容区底改为纯色 `#F5F7FA`，去掉蓝色渐变。
- 五卡 flex + `gap:24px` + `flex:1`；白底、`radius:16px`、`padding:24px`、阴影 `0 10px 30px -8px rgba(15,23,42,0.12)`、淡边框。
- 卡内左右布局：左标题+大数字，右辅助说明（数字 `#3B82F6`）；缓存 `?v=20260719-kpi-float`。

## 2026-07-19（完成 - 侧边栏深蓝渐变）

- 侧边栏改为垂直渐变：`#0F2847` → `#0A1A2F`；文字/图标浅灰白 `#E8EEF5`；选中态半透明青蓝 `rgba(56,189,248,0.18)`。
- 相关页面 `styles.css` 缓存 bump 为 `?v=20260719-sidebar-navy`。
- 浏览器实测：computed style 已生效。

## 2026-07-19（完成 - 登录页标语浅蓝悬浮）

- 登录页 pill 边框/底改为 Soft 浅蓝（`#BFDBFE` / `#EFF6FF`），文字 `#2563eb`。
- 增加 hover 上浮与蓝色阴影，与首页看板风格对齐。

## 2026-07-19（完成 - 登录页能力标语更新）

- 登录页 pill 由「智能招聘 / 权限管理 / 数据分析」改为：AI简历筛选、JD自动匹配、候选状态跟踪、招聘交付闭环。
- pill 增加统一 `min-width` + 居中，便于四条文案排版对齐。
- 契约测试：`tests/test_login_page_markup.py`。

## 2026-07-19（完成 - 月度看板标题加提示）

- 首页「月度数据看板」后文案为 `(点击下面看板查看详情)`；缓存 `?v=20260719-month-hint2`。

## 2026-07-19（完成 - 操作日志 KPI 对齐 Soft）

- `logs.html` 增加 `.logs-page` 作用域，顶部看板接入统一 Soft 浮卡；缓存 `?v=20260719-logs-kpi-soft`。

## 2026-07-19（完成 - 任务/岗位相关页 KPI 统一 Soft）

- 任务看板（通知页）、岗位发布、岗位列表、每日任务顶部看板对齐首页 Soft 浮卡。
- 扩展 `list-pages soft KPI` 作用域；缓存 `?v=20260719-recruit-kpi-soft`。

## 2026-07-19（完成 - 去掉关键词/城市输入图标）

- 候选人页关键词、期望城市输入框内小图标已移除；左内边距恢复为普通输入。
- 缓存 `?v=20260719-cand-no-icons`。

## 2026-07-19（完成 - 关键词框缩短防遮挡）

- 关键词输入 `max-width: 420px`；期望城市固定宽度且不收缩；搜索按钮不再盖住城市框。
- 缓存 `?v=20260719-cand-search-width`。

## 2026-07-19（完成 - 候选人筛选区压缩）

- 关键词搜索与筛选条件合并为一张卡；标签改左侧横排；热词/下拉行去内嵌灰底框。
- 输入与按钮高度约 34px，内边距收紧；缓存 `?v=20260719-cand-filter-compact`。

## 2026-07-19（完成 - 三页顶部 KPI 对齐首页）

- 客户管理 / 项目管理 / 候选人简历池：仅顶部数字看板改为首页同款 Soft 浮卡（白卡软阴影、无图标、大数字黑）。
- CSS 作用域覆盖，不改筛选/表格逻辑；缓存 `?v=20260719-list-kpi-soft`。
- 契约测试：`tests/test_list_pages_kpi_soft_ui_css.py`。

## 2026-07-19（完成 - 日历推荐数加「条」）

- 推荐日历有数据日期由纯数字改为「N条」。
- 缓存 `?v=20260719-calendar-tiao`。

## 2026-07-19（完成 - KPI/月度卡压扁长条）

- 顶部 5 卡与月度 4 卡改为更扁长条（减小 padding/字号/间距），给下方看板腾空间。
- 大数字黑色 `#0F172A`，说明中小数字蓝色 `#3B82F6`。
- 缓存 `?v=20260719-dashboard-compact`。

## 2026-07-19（完成 - KPI 单层白卡悬浮）

- 去掉外层托盘；只保留一层白卡 + 柔和多层外阴影做悬浮。

## 2026-07-19（完成 - KPI 外框去描边）

- 曾去掉外框描边；已进一步取消整层外托盘。

## 2026-07-18（完成 - 顶部 5 卡托盘加深立体）

- 仍只一层底托：底托改为更深浅蓝渐变、底部留白更大；内浮卡阴影加厚增强立体感。

## 2026-07-18（完成 - 月度卡说明数字高亮）

- 月度数据看板说明中的数字（如「共 X 人」）与主数字同色 `#3B82F6`、加粗醒目。
- 复用 `colorizeNote`；缓存 `?v=20260718-dashboard-note-num`。

## 2026-07-18（完成 - 看板框线统一浅蓝）

- 数据看板内各框边线统一为 `#BFDBFE`（`--dashboard-line`）：日历格、月度卡、面板、chip、明细行等。
- 顶部 5 卡仍无描边（悬浮阴影）。缓存 `?v=20260718-dashboard-line-blue`。

## 2026-07-18（完成 - KPI 真悬浮 + 月度卡去图标）

- 顶部 5 卡：去描边、多层柔和阴影、内容区浅蓝底衬托，形成真悬浮。
- 月度数据看板 4 卡去掉小图标。
- 缓存 `?v=20260718-dashboard-float-v2`；契约测试 4 passed。
...
- Controller 目视（`http://127.0.0.1:8000/src/pages/dashboard.html`）：浅蓝渐变底、白卡软阴影、日历实心蓝选中、空态保留。
- 已 push：`bf3192e..b8eb758` → `origin/main`。未改业务 JS / API。
...
- 文件：`src/pages/recruit-job-publish.html`。
...
- 候选人池：3 KPI+绿趋势、搜索图标前缀、中性「猜你想搜」、灰筛选 label、保存方案品牌蓝、状态 pill 归色、推荐禁用浅蓝。
- 评价：单卡 24px、搜索+等级等高、等级 pill、分值单位弱化、空态居中。
- 缓存：`?v=20260715-notices-cand-eval-ui`；折叠箭头改 CSS 旋转，`app.js` 不再改 icon 文本。

## 2026-07-15（完成 - 美化每日任务 + 岗位发布页）

- 每日任务：KPI 语义图标；岗位列表次按钮；日期查询 flex；空态日历图标；页脚浅灰对齐。
- 岗位发布：3 KPI；两栏字段卡表单；发布主按钮；数据源浅蓝 pill；最近发布次按钮+空态。
- 缓存：`?v=20260715-recruit-tasks-publish-ui`。

## 2026-07-15（完成 - 美化客户管理页）

- KPI 白卡+语义图标；新建主按钮/刷新次按钮；公司名蓝链；去掉名称下重复状态标签，状态列绿 pill。
- 项目数/岗位数品牌蓝加粗；操作次按钮+删除危险态；干净表格行+hover 浅蓝。缓存：`?v=20260715-customers-ui`。
...
- 去掉岗位名称下重复标签；缓存 `?v=20260715-projects-ui2`。
...
- Spec：`docs/superpowers/specs/2026-07-15-ui-global-design-system.md`
- 用途：后续 9 个页面美化共用，不再重复粘贴全局要求
...
## 2026-07-15（完成 - 搜索核心词提取）

- 职位通配停用词（工程师/主管/经理等）不参与「任一命中」；复合词只取核心（算法工程师→算法，销售主管→销售）。
- 单独搜通配词不命中；全中仍按整词/全分词；高亮也只标核心词。
- 缓存：`app.js?v=20260715-core-terms`。

## 2026-07-15（完成 - 搜索排序规则修正）

- 新顺序：全中职位 > 全中工作/项目经历 > 职位部分+经历部分 > 仅经历部分 > 仅职位部分。
- 缓存：`app.js?v=20260715-rank-v2`。

## 2026-07-15（完成 - 搜索排序四级优先级）

- 排序改为：全中职位 > 某分词中职位 > 全中工作/项目经历 > 某分词中工作/项目经历；同档比命中子词数。
- 缓存：`app.js?v=20260715-rank-tier`。

## 2026-07-15（完成 - 分词 OR 命中 + 经历摘要高亮）

- 匹配：分词后任一子词命中即入选；排序优先全中组数，再命中子词数，再 L1/L2。
- 展示：工作经历/项目经历/核心优势/证书命中时，列表姓名下展示摘要，`mark` 高亮关键词。
- 缓存：`app.js?v=20260715-segment-or`。

## 2026-07-15（完成 - 分词搜索可用性加固）

- 问题：分词已合入，但 SPA 常缓存旧 `app.js`；且「生鲜运营总经理」对「生鲜运营负责人」因缺「总经理」整句 AND 落空。
- 修复：入口页 `app.js?v=20260715-segment` 强刷缓存；分词全中优先，否则 ≥2 词且命中字数 ≥50% 软匹配；关键词/城市支持回车搜索。
- 本地校验：`生鲜运营总经理` → 命中「生鲜运营负责人 / 生鲜 运营 总经理」，不命中「市场总监」。
...
- 修复：分页监听改单例绑定；全局按钮处理跳过 `prev-page` / `next-page`。

## 2026-07-15（完成 - 简历池去掉搜索区内层框）

- 去掉关键词/期望城市外层边框与色块背景，输入区不再套第二层框；保留输入框与右侧「搜索 / AI检索」。
- 同步移除关键词下方说明文案（`.candidate-search-hint`），进一步压缩顶部高度。

## 2026-07-15（完成 - 简历池去掉搜索工具栏标题）

- 移除「搜索工具栏」标题与说明；「搜索 / AI检索」移到关键词、城市输入行右侧。

## 2026-07-15（完成 - 简历池顶部压缩并取消看板筛选）

- 看板改为静态展示，去掉点击筛选。
- 压缩指标卡/搜索栏/筛选区上下边距与内边距，列表更早上屏。
...
## 2026-07-13（完成 - 日历分组不再把超管当成组）

- `resolve_group_leader`：仅认角色「组长」；找不到则「未分组」。
- 前端禁止用推荐人回填 `groupLeader`；「未分组」文案不再拼成「未分组组」。
- 测试：`tests/test_dashboard_recommendation_calendar.py` 3 项 PASS。

## 2026-07-13（完成 - 首页日历/月度看板同区明细列表）

- 月度看板下方统一明细区：默认空态；点日历日或面试/推荐/入职/新增岗位卡刷新列表。
- 候选人一人一行（去重）；新增岗位展示岗位行（与数字口径一致）；统一行样式 + 清空按钮。
- 涉及：`dashboard.html`、`styles.css`；规格见 `docs/superpowers/specs/2026-07-13-dashboard-detail-list-design.md`。
...
- 修复：改为本页切到「岗位列表」并按紧急程度「高」就地过滤（`filterPositionsByUrgency`）。

## 2026-07-12（完成 - 统一已推荐口径 + 面试自动升已推荐）

- 新增 `recommended_at`；待推荐不计；手工推进或第 N 轮面试自动升已推荐并记时。
- 看板总推荐数 / 本月推荐数按 `recommended_at`；日历改为按该时间。
- 测试：`test_recommended_at_and_interview_promote.py` + 日历/状态联动相关 7 项 PASS。

## 2026-07-11（完成 - 用户列表组长可展开查看组员）

- 组长/主管行姓名下增加「▸ 组员 N」切换；展开显示直属组员面板（用户名/姓名/角色/联系方式/状态/编辑）。
- 样式：`styles.css` 增加 `admin-member-panel` 等。
...
- `users.html`：加载时读 `role` 查询参数，套用现有角色下拉筛选。
...
- 同步：`findings.md` / `task_plan.md` Current Phase 已更新。
...
- 修复：顶部副文案改为同一套月度指标——「本月推荐 N 人」「本月入职 N 人」。

## 2026-07-10（完成 - 任务看板两块默认折叠）

- 「岗位分配待办」「通知列表」默认折叠，点击标题展开/收起。
- `?tab=position-tasks` 进入时自动展开岗位分配待办。
...
- 修复：只跳过 action 名含 `recruit` 的页面自有操作，`logout` 等全局动作在岗位发布/列表/每日任务页可正常执行。

## 2026-07-10（完成 - 左侧导航点击后保持可见，右侧局部刷新）

- 根因：SPA 路由只认已渲染的 `.content`，系统设置等页只有 `__PAGE_BODY__`，会降级整页刷新，侧栏滚回顶部。
- 修复：从 HTML 解析 `__PAGE_BODY__` 并执行页面脚本；只更新右侧内容；侧栏导航区独立滚动，激活项 `scrollIntoView`。

## 2026-07-10（完成 - 全站看板类醒目数字配色）

- 首页：月度数据看板主数字按 tone 着色；推荐日历汇总 chip 与有数据日期条数着色。
- 补齐：操作日志、Recruit 岗位列表/发布/每日任务顶部指标卡 tone。
- 岗位候选人摘要数字区分蓝/绿/橙/灰。

## 2026-07-10（完成 - 客户/项目/简历池/任务页指标卡配色）

- 客户管理、项目管理、求职者数据池、任务看板（通知页）顶部指标卡按首页同款 tone 配色：主数字蓝/绿/橙/红/灰，左侧色条同步。
- 样式落在 `.metric-card.tone-*`，各页仅加 tone class。

## 2026-07-10（完成 - 首页标题与看板数字配色）

- 首页标题由「招聘交付驾驶舱」改为「统计数据看板」，副文案改为核心指标汇总说明。
- 顶部五张看板主数字与副文案中的数字按卡片色调着色（蓝/绿/橙/红/灰），左侧色条同步区分。

## 2026-07-10（完成 - 批量推荐弹窗右侧常显滚动条）

- 用户要求：弹窗内容区右侧（标注位置）要能看到滚动条。
- 修复：表单区固定 `max-height: min(420px, calc(100vh - 220px))` + `overflow-y:scroll`，并加 WebKit/Firefox 滚动条样式，大屏也会显示滚动条。

## 2026-07-10（完成 - 批量推荐弹窗滚动条可见）

- 根因：面板 `margin:12vh` + `max-height:90vh` 超出视口时被遮罩裁切，面板自身不溢出，滚动条不出现。
- 修复：遮罩 `overflow-y:auto`；面板限高 `calc(100vh - 48px)`；表单区 `overflow-y:scroll`，矮屏可滚且滚动条常显。

## 2026-07-10（完成 - 首页月度看板改为漏斗累计口径）

- 问题：只看「当前仍是面试状态」时，本月已入职的人会从面试桶消失，出现「入职 2、面试 0」。
- 修复：按候选人去重统计；面试 = 跟踪面试 ∪ 面试记录 ∪ 当前面试中推荐 ∪ 本月入职；推荐再并入面试集合，保证推荐 ≥ 面试 ≥ 入职。
- 首页额外加载 `candidateTrackingEvents()`。

## 2026-07-10（完成 - 简历池批量推荐弹窗可滚动）

- 问题：矮屏/小视口下「批量推荐至岗位」弹窗底部表单被裁切，无法看到全部字段。
- 修复：`candidates.html` 中该弹窗 `.panel` 增加 `max-height` 与 `overflow-y`（后续已改为表单区常显滚动，见上条）。

## 2026-07-10（完成 - 首页月度看板按推荐状态口径统计）

- 根因：`RecommendationOut` 缺 `created_at/updated_at`，按月过滤失效；面试/推荐卡口径也不对。
- 修复：推荐 API 返回时间戳；月度卡改为按状态统计——面试=`安排面试|面试中`（`updated_at` 本月）、推荐=`已推荐`（`updated_at` 本月）、入职=`已入职`（`onboard_date` 优先）。（后续已改为漏斗累计口径，见上条。）

## 2026-07-10（完成 - 岗位列表候选人数字可跳转过滤）

- 岗位列表「候选人 / 选中 / 未选 / 淘汰」数字改为可点击链接。
- 跳转 `position-candidates.html?position_id=&funnel=`；候选人总数不带 funnel，其余带 `selected|unselected|rejected`。
- 岗位候选人页新增「漏斗分桶」筛选，分桶规则与岗位列表统计一致；进入时自动应用 URL funnel。
...
- 增删改仍仅管理员；新增 `tests/test_tags_read_permission.py`。

## 2026-07-10（完成 - 首页布局方案 A）

- 推荐日历缩小：格子约 54px，标题/导航/详情条同步压缩。
- 首页改为：顶部指标卡；下方左日历 + 右月度看板并排；窄屏自动上下堆叠。

## 2026-07-10（完成 - 统计页改为完整大屏）

- 统计页去掉面包屑、页面标题与说明，仅保留右上角消息铃铛。
- 大屏内容区铺满主区域，消息按钮浮在右上角。

## 2026-07-10（完成 - 猜你想搜与搜索框统一匹配逻辑）

- 热词过滤改为调用 `matchesSearchKeywords` / `buildCandidateSearchText`，与搜索框同为 L1/L2 岗位字段 + 短词边界匹配。
- 有热词时默认排序也按期望岗位命中优先（与搜索框关键词一并参与相关性）。

## 2026-07-10（完成 - 简历池搜索工具栏配色）

- 搜索工具栏增加左侧主色竖条与浅蓝底；关键词区蓝底、城市区青底。
- 「搜索」改为浅蓝描边按钮，与蓝色「AI检索」区分；提示文案关键词高亮。
...
- 简历池「猜你想搜」改为读取启用热词；空库自动种子默认热词。
- 单测 `tests/test_search_hotwords.py` 通过。

## 2026-07-09（完成 - 简历池筛选区配色区分）

- 热词行浅蓝底 + 蓝/青/琥珀轮换标签，选中为实心主色。
- 下拉行浅灰底，标签分色；「保存搜索方案」改为绿色实心按钮。
- 面板左侧主色竖条，避免整片灰白。

## 2026-07-09（完成 - 简历池筛选区压成两行）

- 「猜你想搜」与热词同一行；学历/年龄/经验/排序/状态下拉与「保存搜索方案」同一行。
- 去掉原先左右双面板大标题，减少纵向占位。

## 2026-07-09（完成 - 修关键词 L1：职位名优先 + 短词边界）

- L1 纳入 `current_title`；短词 `ai` 不再误命中 `AIGC`；列表「期望职位」列展示意向或职位名。
- 单测 7 passed。
...
## 2026-07-09（完成 - 「我的项目」勾选移至列表标题栏）

- 勾选框放在项目列表面板标题栏右侧（新建项目按钮左侧），搜索栏不再占位。

## 2026-07-09（完成 - 「我的项目」勾选移至项目名称下方）

- 去掉说明小字；勾选框缩小并放在项目名称输入框下方，仅保留「我的项目」标签。

## 2026-07-09（完成 - 项目列表「我的项目」勾选过滤）

- 搜索栏增加「我的项目」复选框；后端 `/api/me/assigned-project-ids` 按岗位分配解析项目 ID；`findings.md` 已同步。

## 2026-07-09（完成 - 简历池搜索框语法提示）

- 搜索框下方增加小字说明：空格=且，逗号/竖线=或，并附示例 `Java,Spring 北京`。

## 2026-07-09（完成 - 简历池搜索支持 OR 语法）

- 组内 `,` / `，` / `|` 表示或；空格仍表示且；例 `Java,Spring 北京` = (Java 或 Spring) 且 北京。
- `task_plan.md` 已同步搜索语法说明。

## 2026-07-09（完成 - 简历池多关键词 AND 搜索）

- 搜索框空格分隔多词，全部命中才展示（智联/前程口径）；扩展至工作经历、项目、证书等全文；后端 `list_candidates` 同步。

## 2026-07-09（完成 - 岗位弹窗去掉薪酬规则）

- 创建/编辑岗位弹窗移除「薪酬规则」下拉；薪酬仅用上方 `salary_min` / `salary_max`；`requirement_tags` 不再写入 `salary` 字段，不改库表结构。
...
## 2026-07-09（完成 - 已入职校验回退面试评价岗位）

- 「已入职」开关不再仅依赖薪资记录；会依次从约定薪资、面试评价、推荐岗位、当前页岗位解析公司与岗位名。

## 2026-07-09（完成 - 岗位候选人展开面板样式与评价文案）

- 展开区改为双栏卡片布局；评价有等级/分数时不再显示「暂无评价内容」，仅在有文字备注时展示内容行。
...
- 面包屑：`app.js` 增加 `crumbHrefMap` / `renderCrumbsHtml`，上级路径可点击返回。
- 涉及文件：`app.js`、`styles.css`、`position-candidates.html`、`findings.md`、`task_plan.md`、`progress.md`。

## 2026-07-08（完成 - 面包屑上级可点击跳转）

- 全站顶部面包屑除当前页外，上级路径均可点击返回对应列表页（如「项目管理 / 岗位候选人」中「项目管理」跳转 `projects.html`）。

## 2026-07-08（完成 - 岗位候选人展开行空态文案）

- 「候选人跟踪」「候选人评价」两块始终展示；无数据时分别显示「无跟踪记录」「无候选人评价」（系统 AUTO_MATCH 仍不计入跟踪）。

## 2026-07-08（完成 - 岗位规则、JD 与候选人页一批改造）

- **岗位与项目规则**：锁定上限 10、去掉创建时自动匹配、项目招满自动「招聘完毕」、简历池「未面试/自己锁定」筛选、客户跳转项目。
- **岗位 JD**：复用 `positions.description`；创建/编辑弹窗、岗位列表摘要、候选人页完整展示。
- **岗位候选人页**：副标题改为动态「客户 · 项目 · 岗位」；移除处理时限；筛选与列表合并为单卡片布局。
- **推荐权限**：未锁定简历池候选人推荐不再校验录入人归属，仅校验岗位权限。
- **其他**：展开面板改「候选人评价」；任务看板导航分组调整。
- **测试**：`test_position_lock_quota_and_project_complete.py`（3 passed）、`test_operator_can_recommend_unlocked_candidate_owned_by_peer` 通过。

## 2026-07-08（已完成 - 简历池推荐不再校验候选人归属权限）

- 根因：单条/批量推荐仍调用 `enforce_candidate_access`，操作员无法推荐他人录入的未锁定简历。
- 修复：新增 `assert_can_recommend_candidate`——未锁定候选人视为公共简历池，仅校验目标岗位权限；已锁定仍走原归属校验（且会被「已锁定」拦截）。
- 测试：`test_operator_can_recommend_unlocked_candidate_owned_by_peer` 通过。

## 2026-07-08（已完成 - 岗位候选人展开面板改为「候选人评价」）

- 岗位候选人列表展开行右侧标题由「面试 / 薪资 / 入职」改为「候选人评价」。
- `fetchCandidatePanels` 改为读取 `/api/evaluations` 展示轮次、岗位、等级、分值、内容与评价人，与详情页评价录入数据一致。

## 2026-07-08（已完成 - 任务看板移到「主要功能」分组末尾）

- 将侧栏「任务看板」从「平台岗位发布」移到「主要功能」分组最后一项，便于从主要业务入口直接进入岗位待办。

## 2026-07-08（已完成 - 岗位与项目规则改造）

- **创建岗位**：去掉关键词搜索/自动匹配；锁定名额固定 10；新增客户下拉，选中后展开该客户下项目二级选择。
- **锁定规则**：后端单岗位推荐锁定上限 10；达到上限禁止新增；移除推荐后可补入。新增测试 `tests/test_position_lock_quota_and_project_complete.py`。
- **客户列表**：每行增加「项目」按钮，跳转项目管理页并按 `company_id` 过滤。
- **简历池**：状态过滤增加「未面试」；候选人列表返回 `has_interview_round`。
- **项目完成**：按「每个岗位已入职人数 == 应招人数」判定招聘完毕；入职写入时同步项目状态，列表 `_project_out` 实时校准展示。
- **文案**：导航分组「岗位管理」→「平台岗位发布」。
- **验证**：`node --check app.js` 通过；`pytest tests/test_position_lock_quota_and_project_complete.py` 3 passed；相关推荐/入职回归 8 passed。
...
- **流程状态配置按钮迁移与锚定**：将“薪资记录”、“入职状态”、“随访记录”三项展示/跟进按钮从候选人编辑资料浮窗移除，调整至候选人详情浮窗头部右侧（actions区域）。其中，“入职状态”与“随访记录”维持原先的弹窗与定位逻辑，“薪资记录”按钮的点击事件重构为将视图平滑滚动并锚定到下方的“薪资/福利/入职条件跟踪表”区域，避免了原先误弹出添加表单的体验。
  * **优化随访已录用/已入职状态判定**：修复了原随访校验仅判断 `candidate.status === '已录用'` 的漏洞（因为在数据库中该字段仅表示锁定状态，实际录用/入职状态存储在推荐记录或入职状态表中，导致已入职的周靖博等人被错误拦截）。重构判定逻辑，只要候选人自身状态、任意推荐关联状态或存在已入职记录满足录用/入职条件，即放行通过。

## 2026-06-30（已完成 - 以角色为中心的一站式权限配置与岗位管理菜单补齐）


- **后端权限管理 API**：
  - 新增了 `DELETE /api/role-permissions` 角色权限物理删除接口，支持在解除关联时对数据表执行精准删除和审计日志写入。
- **前端功能权限多选勾选改造**：
  - 功能权限管理页面中，将“关联角色”的选择由单选下拉框改为了多选 Checkbox 复选框，打开时自动聚合角色，保存时智能通过 N-O 差集计算并行并发 Promise 提交保存和解绑。
  - 功能权限的“权限标识”输入框升级为带有“快捷选择”选项的可视化下拉菜单，选择功能时自动联动填充类型和所属模块，极大降低了非技术管理员的手动配置成本。
  - 隐退并移除了“权限管理”页面右上角多余的“+ 添加权限”按钮，功能页面仅用于全局只读矩阵展示和单项修改。
- **角色管理页面一站式授权改造**：
  - 重构了角色管理的操作列表，去除了旧有的“配置权限”跳转链接，统合为单一的“编辑与授权”弹窗。
  - 在编辑角色弹窗的右侧增加可滚动的“功能权限分配”网格，平铺展示系统所有的页面和操作权限复选框，并且通过与底层 `role_permissions` 表的直接绑定实现与权限管理矩阵的双向实时联动。
- **岗位管理菜单补齐与调研**：
  - 在权限标识预设字典和名称翻译中补齐了“岗位发布 (`page:recruit-job-publish`)”、“岗位列表 (`page:recruit-job-list`)”和“每日任务 (`page:recruit-daily-tasks`)”三项，支持对这三项进行独立的权限分发。
  - 完成对 `recruit` 模式下数据表字段与操作人属性绑定的审计，确认岗位表 `job_postings` 包含 `employee_id`，每日统计表 `daily_task_stats` 和下载表 `resume_downloads` 均包含 `issuer_login_name`。
- **回归测试验证**：
  - 跑通 pytest 所有 22 个自动化回归测试用例，结果全量通过且环境无脏数据残留。

## 2026-06-30（已完成 - 数据权限隔离体系大版本开发：全员共享明文人才库 + 过程隐私阻断 + CTE递归下属树）

- **物理模型升级**：为推荐、面试、薪资、入职在库表增加 `user_id` 相关外键并完成 DDL 表结构升级。
- **递归组织层穿透**：改写 `subordinate_user_ids` 为 Postgres CTE 递归查询模式，完美支持超过三层无限深度的上下级穿透可见。
- **公共人才库共享开放**：移除人才库中对 Candidate 列表和详情的 owner 拦截限制，使所有顾问共享公司完整明文人才资源（手机号及邮箱字段不脱敏明文呈现）。
- **过程流程同级隔离**：对这几类候选人身上发生的岗位推荐进度、面试记录、Offer 约定和薪金进行了基于 `operator_user_id` 的同级彻底阻断隔离，但对其上级组长和总监保留无感穿透可见。
- **质量保障与测试**：新增 `test_data_permissions_isolation.py` 全功能用例，重构更新了既有的 RBAC 和 Smoke 用例，22 个测试用例全部 100% Passed。
...
- **评价体系 (`evaluations.html`)**：为“优秀”、“良好”、“待复盘”卡片增加就地过滤，点击可快速切换并渲染对应等级的评价记录；为“评价总数”卡片绑定重置条件。
- **操作日志 (`logs.html`)**：为“导入日志”、“推荐日志”、“权限变更”卡片绑定就地过滤，点击自动修改“模块”检索条件并就地局部重绘操作明细；“操作总数”卡片绑定重置。
- **通知列表 (`notifications.html`)**：为“未读通知”卡片绑定就地过滤，点击可即时过滤出所有未读通知记录，汇总卡片支持重置。
- **Recruit 岗位列表 (`recruit-job-list.html`)**：为“有效岗位”、“跳过岗位”卡片绑定就地状态过滤；并支持在 DOM 加载时解析 URL 传入的 `valid` 查询参数以实现跨页过滤初始化。
- **Recruit 岗位发布 (`recruit-job-publish.html`)**：为“Recruit 岗位”、“有效岗位”、“今日任务”卡片绑定了跨页跳转能力，分别导流至关联的列表页（带参数）和每日任务页。
...
- 为“岗位总数”与“招聘人数”卡片绑定了点击清空过滤条件的交互；并为“紧急岗位”卡片配置了点击就地实时过滤的功能，点击后直接在页面内部向 `app.js` 注入 `urgency: '高'` 条件，无需重刷页面即可即时呈现紧急岗位列表。
...
<!-- trigger update for pre-commit (v45) -->
...
## 2026-07-01 岗位管理功能升级
- 重新设计了『新建岗位』的前端 UI 弹窗，支持选择一系列岗位要求标签（如年龄、性别、学历、经验等）和目标简历数。
- 后端增加了 `requirement_tags` 和 `target_resume_count` 字段，并在创建岗位后增加了自动匹配简历的逻辑。
- 自动匹配：根据岗位要求标签匹配最多 N 个（默认 10 个）候选人，锁定候选人并生成推荐记录，实现资源的有序分配与防占坑。


- 删除了独立的 `positions.html`（岗位管理）页面，并将侧边栏的岗位管理菜单移除。新建和管理岗位的核心功能统一保留在【项目管理-岗位列表】中，确保代码简洁无冗余。


- 移除了岗位列表中的【分配权限】按钮以精简操作面板。
- 重新设计并同步了【编辑岗位】弹窗，使其在样式、排版及新加的字段（关键词搜索、拆分薪资、城市级联、年龄/性别/学历/经验/薪资/状态等要求标签、目标简历数）上，与【创建岗位】保持完全一致。
- 扩展了城市选择联动组件使其支持多个弹窗实例，并更新了后端 `PositionUpdate` 验证模型以确保编辑后的新字段正常持久化保存到数据库。


## 2026-07-03

- Task finalized by Codex hook (unknown) at 2026-07-03 08:05:21

## 2026-07-05

- Task finalized by Codex hook (unknown) at 2026-07-05 17:37:38
- 重构了评价体系页面（src/pages/evaluations.html），移除指标卡片、等级统计与历史摘要，只保留统一的【评价记录】表格。
- 为评价记录表格新增了候选人姓名搜索框与评价等级下拉过滤筛选器。
- 优化了表格布局结构，字段完全对齐：候选人、关联岗位、关联轮次、等级、分值、评价内容、评价人、评价时间。
- 对接并同步了前端 API，更新了 app.js 中新增评价成功后的即时重新渲染逻辑，确保表格数据自动刷新而无需强制刷新页面。


- 重构了候选人详情弹窗中的【薪资/福利/入职条件跟踪表】为【面试评价记录】（同步修改 src/pages/position-candidates.html 和 src/pages/warranty.html）。
- 替换了对应的添加弹窗 UI 和属性映射，改为填写：面试轮次、关联岗位、评价等级、评分和评语备注。
- 在后端 main.py 和前端 API 封装中实现了对于 Evaluation 的 PUT（更新）与 DELETE（删除）接口，使用户可以在详情页增删改面试评价记录。
- 优化了前端关联绑定逻辑，在添加/编辑/删除评价保存后，通过详情页接口自动重新拉取，在详情弹窗内无缝动态局部重绘更新评价列表。


- 优化了候选人详情中新增评价的联动填充逻辑。
- 新增评价时，优先读取页面上下文的 URL 查询参数（如岗位候选人页面的岗位 ID）自动预设岗位。
- 异步拉取候选人关联的所有面试跟踪历史，自动分析并解析出最新一条有效的【岗位 ID】与【面试轮次】，并在打开评价弹窗时自动预选对应的值，无需用户手动拉取或重复点选。


- 移除了评价体系页面（src/pages/evaluations.html）顶部的【新增评价】按钮。
- 修改了候选人详情面板头部的导航按钮（同步修改 src/pages/position-candidates.html 和 src/pages/warranty.html）：将【薪资记录】按钮文本更名为【评价记录】，点击可直接平滑滚动定位到下方的面试评价记录模块。
- 移除了候选人详情/编辑模态框头部的【随访记录】按钮。


- 修复了候选人详情弹窗中新增或删除评价后，列表未实时刷新的问题。
- 优化了 app.js 中对当前激活候选人 ID 的多重获取策略（从事件源、DOM 属性及 body dataset 中进行兜底获取），彻底解决了由于模态框 dataset 关联不匹配导致无法自动触发 view-detail 重绘更新的 Bug。


- 彻底重构了全局面板渲染函数 window.updateCandidatePanels 中的数据加载与 DOM 渲染逻辑。
- 将原本加载的薪资记录（salaryRecords）更替为真实的面试评价记录（evaluations），并配套修改了表格中 8 列的渲染格式（面试轮次、关联岗位、等级、分值、评价内容、评价人、评价时间、修改/删除操作按钮），彻底排除了旧薪资渲染逻辑覆盖评价记录渲染的致命 Bug。


- 调整了侧边栏（app.js）主要功能分类中的菜单项顺序为：首页、客户管理、项目管理、求职者数据池、简历导入、统计管理、评价体系。


- 限制了操作员角色（OPERATOR）创建岗位的权限（前端隐藏【新建岗位】与【JD单生成岗位】按钮，后端 add_position 接口抛出 403 错误）。
- 重新为超级管理员与组长角色提供了岗位列表行的【分配权限】操作按钮，并扩宽表格操作列为 210px 保证排版整齐；操作员仅能查看到“只读岗位”标识，不可编辑或删除他人岗位。
- 组长分配岗位给操作员时，前端会弹出人员选择框（组长仅能查看名下的直属操作员），点击确定后会将岗位的 owner_user_id 修改为该操作员。
- 后端 enforce_position_access 机制加入了针对操作员的安全检测：操作员只能推荐（投递）候选人到明确分配给其自身的岗位，如果企图投递到属于他人或公共的岗位，接口将抛出 403 错误。
- 修复了 GET /api/users 角色过滤规则，操作员请求该接口将正常返回 403 Forbidden，超级管理员和组长仍能继续正常使用，已全部通过 25 项系统测试。


- 调整了【分配权限】逻辑为多选（Checkbox），允许组长将同一个岗位分配给多个组员；弹窗时会自动回显该岗位当前已分配的组员。
- 限制分配范围为组长管辖下的直属组员列表，不用考虑超级管理员。
- 创建岗位和分配权限逻辑仅组长（LEADER）和超级管理员（作为系统管理降级兜底）可执行。
- 后端 DataPermission 查询和校验机制完美支持一个岗位对应多名操作员的数据权限判定。
- 已运行自动化测试并通过了全部 25 项系统测试。


- 修复了客户（Company）、项目（Project）、岗位（Position）的可见性规则，现在这三者对于所有角色（超级管理员、组长、操作员）均是全局完全可见的，确保团队顺畅查看和流程衔接。
- 修复了“新建岗位”弹窗中项目下拉列表没有实时重新加载而导致的页面缓存问题。每次点击【新建岗位】时会动态调用后端最新的项目接口填充下拉菜单。
- 保持并细化了操作员（OPERATOR）只读和推荐权限隔离：虽然项目与岗位对操作员可见，但操作员只能投递候选人到明确分配给他们自己责任范围内的岗位，若企图投递到未授权岗位仍被拦截。
- 重新整理了推荐数据隔离和导出权限规则，已完全通过 25 项系统测试。



## 2026-07-07

- 重构了评价体系页面并新增了搜索与过滤筛选，移除了冗余的随访和薪资板块。
- 限制操作员岗位创建权限，并在后端 API 进行严格的 403 权限拦截。
- 实现了组长多选指派岗位人员（Checkbox 形式）并在指派权限弹窗中自动回显当前分配人员列表。
- 放开了公司、项目、岗位的可见性判断，对于组长和操作员均全局完全可见。
- 精细化了操作员推荐与导出权限精确拦截核验，成功通过 25 项系统测试。
- 优化了批量推荐的即时提示信息，现在会在弹窗下方的 Toast 提示里明确显示失败及跳过候选人的具体原因（例如“无权访问该候选人”、“候选人已锁定”等），提升用户体验。
- 修复了新建岗位自动匹配候选人为 0 的问题。将前端新建岗位弹窗中的“学历要求”和“工作经验”默认选项从之前的“本科”与“应届生”修改为“不限”，避免了在用户仅期望过滤岗位关键字时，系统由于默认的高学历/高经验限制导致匹配结果为空的问题。
...
## 2026-07-08（完成 - 首页推荐日历按角色统计推荐人数）

- 保留首页现有“推荐日历”组件，不增加卡片或新模块；删除原来优先统计交付记录、再兜底推荐记录的逻辑。
- 新增首页专用推荐日历取数接口，仅返回推荐表中状态为“已推荐”的记录，并按推荐记录创建日期计入每天数字。
- 操作员只读取本人推荐记录；组长读取本人及递归下属组织范围内全部推荐记录；超级管理员读取全部推荐记录。
- 新增角色范围测试，覆盖组长本人、直属操作员、外组操作员和“待推荐”排除规则。
- 验证通过：完整 `pytest -q`（28 passed）、Python/JavaScript 语法和 `git diff --check`；浏览器确认原日历正常渲染“X条”和“本月累计推荐”，控制台无错误。

## 2026-07-08（完成 - 日历分组详情与本月新增岗位）

- 点击日历日期后的详情改为按组长汇总，显示“组长名组 推荐 N 人”。
- 月度数字看板最后一项由“本月客户反馈”改为“本月新增岗位”，按岗位创建时间统计当前展示月份。
- 轻量验证通过：推荐日历定向测试 1 passed、Python/JavaScript 语法及差异检查通过。

## 2026-07-08（完成 - 岗位候选人页布局重排）

- 合并筛选区与列表区为单一卡片，采用与项目页一致的 `section-head`：标题左、操作按钮右。
- 筛选条件改为整行横向排列，表格表头独立一行，修复此前左右分栏导致的空白与错位。

## 2026-07-08（完成 - 岗位候选人页布局调整）

- 「岗位候选人列表」标题移至筛选卡片顶部；「添加候选人」「导出简历」移至表格表头右侧。
- 移除「剩余 2h 处理时限」标签；客户/项目/岗位信息上移至页面主标题副文案，去掉原「管理该岗位下的推荐候选人。」。

## 2026-07-08（完成 - 岗位 JD 字段）

- 复用 `positions.description` 字段，创建/编辑岗位弹窗新增「岗位 JD」文本框（`projects.html`、`customers.html`）。
- `app.js` 创建、编辑、回填与列表渲染已接入 `description`；岗位列表每条记录顶部展示 JD 摘要，岗位候选人页在摘要上方展示完整 JD。
- 后端无需新增字段或接口改动。

## 2026-07-08（完成 - 候选人“自己锁定”筛选）
...
- **页面及验证**：
  ...
  * 将“岗位名称”列接通词元提取高亮渲染，搜寻命中处自动呈现黄色醒目高亮。
