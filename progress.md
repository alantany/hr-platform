...
## 2026-06-28 (已完成 - 解决质保页“查看详情”报错 & 动态提取复用详情 Modals)

- **动态加载候选人 Modals 双向复用**：
  * **问题定位**：点击质保页的“查看详情”报错 `null is not an object`。这是因为只有 `candidates.html` 中硬编码了候选人详情模态框 `[data-candidate-detail-modal]` 及其关联的子弹窗 HTML。而在质保期页面等其他页面渲染时并没有这些 HTML 元素。
  * **解决方案**：在 `app.js` 的 `view-detail` 事件监听器开始处，当检测到 DOM 中缺失 `[data-candidate-detail-modal]` 时，使用异步 `fetch('candidates.html')` 拉取求职者数据池页面，并使用 `DOMParser` 解析出所有的关联模态框（详情窗、编辑窗、邮件窗、备注窗、面试跟踪窗、约定薪资窗、跟进记录窗），将其动态追加至 `document.body`。
  * **效果**：实现了 100% DRY (不重复编码)。现在在质保页点击“查看详情”不仅能完美弹出详情，且所有内嵌的“编辑资料”、“发送邮件”、“添加面试记录”、“添加约定薪资”以及“入职状态切换”功能均能在质保页面就地直接完美运行，交互完全对齐。
