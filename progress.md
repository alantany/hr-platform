...
## 2026-06-28 (已完成 - 树节点展开 loading 移至右下角 + 按钮间距与列对齐完美优化)

- **UI 与交互细节打磨**：
  1. **展开 Loading 移至右下角**：原先点击树节点（客户/项目/岗位）的展开/折叠箭头时，使用的是 `withButtonBusy` 导致小箭头内部被强行插入 “处理中...” 文本和加载图标，导致按钮极度拉伸形变、文字错位重叠。现将 `treeToggle` 动作从 `shouldShowButtonBusy` 排除，改用全局 `showLoadingToast("正在加载数据...")`，在页面右下角统一呈现加载浮层，加载完成后自动销毁，排版回归清爽。
  2. **候选人专有操作按钮深度拉开**：为了更好地划分“岗位管理操作”与“候选人推荐操作”，在岗位行的 `.table-actions` 内部将操作按钮分为两组，中间的分隔竖线（`border-left`）的左右边距从 `4px` 拓宽至 `24px`。使得 **“添加候选人”** 与 **“批量移除”** 两个按钮明显向右侧偏移，避免视觉误导。
  3. **列表元素全对齐（统一表格网格列）**：原先树结构每一级节点内部各子项使用 flex 布局的 `justify-content: space-between` 分布，由于文案长度不一，导致后续的“操作按钮组”和“状态 chip”在上下行之间完全无法垂直对齐。现实施如下改造：
     * 为 Company, Project, Position, Candidate 所有树节点的主体 title/meta 信息外层 div 赋予统一的 `flex: 1; min-width: 0;`。
     * 将这四类树节点的操作按钮组 `.table-actions` 容器宽度强制限制在固定的 `420px` 并设置统一的 `margin-right: 16px`。
     * 对右侧状态 `chip` 统一定宽为 `80px` 且内容水平居中。
     * 改造后，树状列表的所有动作按钮垂直起点完全对齐，状态标签亦在右侧侧边完全垂直对齐成列，达到真正的表格对齐效果。
