...
## 2026-07-28（完成 - 搜索关键词语法逻辑规则提示文案上线）

- **搜索关键词语法逻辑规则提示卡片**：
  - 在【搜索关键词】输入框正下方新增极简蓝色高亮提示卡片：`💡 逻辑规则说明：关键词之间的空格表示【同时满足】(AND)，使用顿号、表示【满足一个即可】(OR)`。
  - 增强视觉引导，方便操作人员与 HR 精准配置搜索组合。
- **搜索关键词【🔴 硬性必备词】与【🟢 加分优先词】分类呈现**：
  - **后端 AI 解析升级**：DeepSeek LLM 直接输出 `search_keywords` 结构化分组 (`must` 硬性必备词 vs `preferred` 加分优先词)。
  - **前端面板可视化**：右侧【岗位画像】最下方的搜索关键词面板拆分为 **🔴 硬性搜索词 (必备门槛)** 与 **🟢 加分搜索词 (优先匹配)** 两组高质彩色标签云。
  - **交互体验保持**：无论是硬性词还是加分词，点击标签均可**一键自动追加填入**至左侧搜索框。
- **保留与强化双层精筛机制**：硬性要求一票否决门槛 vs 优先要求打分加权。
