...
## 2026-07-28（完成 - 搜索关键词硬性必备词与加分词分类呈现与落地）

- **搜索关键词【🔴 硬性必备词】与【🟢 加分优先词】清晰分组**：
  - **后端 AI 解析升级**：DeepSeek LLM 直接输出 `search_keywords` 结构化分组 (`must` 硬性必备词 vs `preferred` 加分优先词)，且兼顾向前兼容性。
  - **前端面板可视化**：右侧【岗位画像】最下方的搜索关键词面板拆分为 **🔴 硬性搜索词 (必备门槛)** 与 **🟢 加分搜索词 (优先匹配)** 两组高质彩色标签云，视觉上一目了然。
