...
## 2026-07-28（完成 - JD 提取提炼精炼短词与【无则填无】防错规则落地）

- **AI 大模型提炼规则重构 (极简短词 & 严禁整段复制)**：
  - 更新 `crud.py` 中的 System Prompt，明确限定 `工作年限`、`获奖与优先项` 等字段**必须提炼为 15 字以内的极简要点短词**（如 `"8年+数据库运维"`），严禁把 JD 原文整段长句原封不动贴进来。
- **【无则填无】防误导规则 (Strict N/A Rule)**：
  - 指示大模型：若 JD 中没有提到相关的奖项或特殊能力，**必须显式填 "无"**；绝对禁止脑补臆造或硬塞无关的冗余整句。
  - 后端增加 30 字符安全截断与数据清洗防护。
- **搜索关键词【🔴 硬性必备词】与【🟢 加分优先词】分类呈现**：
  - 后端 AI 输出 `search_keywords` 结构化分组 (`must` 硬性必备词 vs `preferred` 加分优先词)。
  - 前端面板可视化拆分为 **🔴 硬性搜索词 (必备门槛)** 与 **🟢 加分搜索词 (优先匹配)**。
- **自动化测试**：全量单元与契约测试全部通过（`2 passed`）。
